...
<div {{ $attributes->merge(['class' => 'group bg-white rounded-2xl border border-gray-200 overflow-hidden hover:border-primary/50 hover:shadow-xl hover:shadow-primary/5 transition-all']) }}>
    <div class="relative aspect-square bg-gray-50 overflow-hidden">
        ...
        @if ($onSale && $originalPrice)
            ...
            @endphp
            @if ($discount > 0)
                <span class="absolute top-3 left-3 bg-secondary text-white text-xs font-bold px-3 py-1.5 rounded-xl">
                    -{{ $discount }}% OFF
                </span>
            @endif
        @endif
        ...
        @if (!$inStock || ($stockQuantity !== null && $stockQuantity <= 0))
            <div class="absolute inset-0 bg-white/80 backdrop-blur-sm flex items-center justify-center">
                <span class="bg-dark text-white text-sm font-semibold px-4 py-2 rounded-xl">
                    Out of Stock
                </span>
            </div>
        @endif
        ...
        @if ($stockQuantity !== null && $stockQuantity > 0 && $stockQuantity <= 5)
            <span class="absolute bottom-3 left-3 bg-orange-500/10 text-orange-600 text-xs font-semibold px-3 py-1.5 rounded-xl">
    ...
    <div class="p-4 flex flex-col">
        ...
        @if ($reviewsCount > 0 || $rating > 0)
            <div class="flex items-center gap-1.5 mb-3">
                @for ($i = 1; $i <= 5; $i++)
                    <svg class="w-4 h-4 {{ $i <= $rating ? 'text-secondary' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="mt-auto">
            @if ($inStock && ($stockQuantity === null || $stockQuantity > 0) && $id)
                <form action="{{ route('cart.add') }}" method="POST" class="add-to-cart-form">
                    ...
                    <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-primary text-white text-sm font-semibold rounded-xl hover:bg-primary-dark hover:shadow-lg hover:shadow-primary/25 transition-all">
