    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if ($count > 0)
            <form action="{{ route('checkout.store') }}" method="POST"
                  x-data="checkoutForm"
                  @submit="document.getElementById('place-order-btn').disabled = true">
                @csrf
                    ...
                    <div class="lg:col-span-2 space-y-6">
                                    ...
                                    @switch($slug)
                                        @case('ozow')
                                            <label class="flex items-start gap-3 p-4 rounded-lg border transition-all duration-200 cursor-pointer bg-white"
                                                ...
                                                <input type="radio" value="ozow" class="mt-1 text-primary focus:ring-primary" x-model="payment_method">
                                                <div class="flex-1">
                                        ...
                                        @case('payflex')
                                            <label class="flex items-start gap-3 p-4 rounded-lg border transition-all duration-200 cursor-pointer bg-white"
                                                   :class="payment_method === 'payflex' ? 'border-primary ring-1 ring-primary' : 'border-gray-200 hover:border-gray-300'">
                                                <input type="radio" value="payflex" class="mt-1 text-primary focus:ring-primary" x-model="payment_method">
                                                <div class="flex-1">
                                                    <div class="flex items-center gap-2">
                                                        <span class="text-sm font-medium text-gray-900">Pay in 4</span>
                                                        <span class="text-xs px-2 py-0.5 bg-teal-100 text-teal-700 rounded font-medium">Payflex</span>
                                                    </div>
                                                    <p class="text-xs text-gray-500 mt-1">{{ $description }}</p>
                                                    <p class="text-xs font-semibold text-primary mt-1">
                                                        4 payments of R{{ number_format($total / 4, 2) }}
                                                    </p>
                                                </div>
                                            </label>
                                            @break
                                        ...
                                        @case('payjustnow')
                                            <label class="flex items-start gap-3 p-4 rounded-lg border transition-all duration-200 cursor-pointer bg-white"
                                                ...
                                                <input type="radio" value="payjustnow" class="mt-1 text-primary focus:ring-primary" x-model="payment_method">
                                                <div class="flex-1">
                                                    <div class="flex items-center gap-2">
                                                    ...
                                                    <p class="text-xs text-gray-500 mt-1">{{ $description }}</p>
                                                    <p class="text-xs font-semibold text-primary mt-1">
                                                        3 payments of R{{ number_format($total / 3, 2) }}
                                                    </p>
                                                </div>
                                            </label>
                                            @break
                        ...
                        <div class="flex flex-col sm:flex-row gap-4">
                            ...
                            <button type="submit"
                                ...
                                Place Order — <span x-text="'R' + total.toFixed(2)">R{{ number_format($total, 2) }}</span>
                            </button>
                        </div>
                    </div>
                        ...
                        <div class="bg-gray-50 rounded-xl border border-gray-200 p-6 space-y-4 sticky top-24">
                            ...
                            <div class="border-t border-gray-200 pt-4">
                                <div class="flex items-center justify-between">
                                    ...
                                    <span class="text-xl font-bold text-primary" x-text="'R' + total.toFixed(2)">R{{ number_format($total, 2) }}</span>
                                </div>
                            </div>
